...
pdftitle={psychedelia syndrome: the pixels and code of a new kind of videogame},                           
...
\usepackage{fancyhdr}                                                   
% Remove 'Chapter N' from the headers.
...
\definecolor{boxblue}{HTML}{beb7f1}
...
\ifstrempty{#1}%
  ...
  \tikz{[baseline=(current bounding box.east),outer sep=0pt]
  \node[font={\small},draw = black, line width = 0.5pt, text = black, anchor=east,rectangle,
  fill=boxblue, rounded corners]
  {\strut #1};
  }}}}%
  \begin{mdframed}[backgroundcolor=boxblue]\relax%
}{\end{mdframed}}
...
\subfile{src/fullgame_listing} % draft
...
\subfile{src/dials} % draft
\subfile{src/aftereffects} % draft
%
\subfile{src/preface_colorspace} %draft
\subfile{src/colorspace_painting} % draft
\subfile{src/colorspace_patterns} % draft
\subfile{src/colorspace_presets} % draft
\subfile{src/colorspace_colourflow} % draft
%\subfile{src/colorspace_explosions} % draft
%\subfile{src/colorspace_ooze} % draft
