...
This is the first line in \icode{starOneXPosArray\index{starOneXPosArray}} and the first line in \icode{starOneYPosArray\index{starOneYPosArray}} listed
...
is illustrated in the table below where we show how this first pair of lines from \icode{starOneXPosArray\index{starOneXPosArray}} and
\icode{starOneYPosArray\index{starOneYPosArray}} is plotted. The numbers given in the middle of the table represent the order that the
...
\icode{starOneXPosArray\index{starOneXPosArray}} and \icode{starOneYPosArray\index{starOneYPosArray}}:
...
for example the points plotted by the first line in \icode{starOneXPosArray\index{starOneXPosArray}} and \icode{starOneYPosArray\index{starOneYPosArray}} are shown 
...
\clearpage
\textbf{Lines 1017-1052. \icode{\textbf{pixelXPositionLoPtrArray, pixelYPositionLoPtrArray}}} 
...
\textbf{Lines 1017-1033. \icode{\textbf{pixelXPositionLoPtrArray/pixelXPositionHiPtrArray}}:} Psychedelia
...
Y Position data structures for the 'Star One' pattern in \icode{starOneXPosArray\index{starOneXPosArray}} and 
\icode{starOneYPosArray\index{starOneYPosArray}}, index 1 will allow us to reference the data structures for 'The Twist' pattern,
and so on.
...
\textbf{Lines 1033-1052. \icode{\textbf{pixelYPositionLoPtrArray/pixelYPositionHiPtrArray}}:} As for the X position array,
...
\clearpage
\textbf{Lines 251-321. \icode{\textbf{PaintStructureAtCurrentPosition\index{PaintStructureAtCurrentPosition}}}} 
...
\textbf{Lines 251-321. \icode{\textbf{PaintStructureAtCurrentPosition\index{PaintStructureAtCurrentPosition}}}:} We've already encountered this routine
...
The pattern the user has selected is stored as an index value in \icode{patternIndex\index{patternIndex}}. We use it to fetch the element
from each array:
...
Imagine our \icode{patternIndex\index{patternIndex}} is \icode{1}. This will give us the following values: 
...
\icode{theTwistXPosArray\index{theTwistXPosArray}} and \icode{theTwistYPosArray\index{theTwistYPosArray}} data structures is to treat each element
as an offset from the cursor's current X/Y position. So that means taking \icode{previousCursorXPosition\index{previousCursorXPosition}}
...
incremented at each iteration) \icode{ADC} is able to point to the next byte in the \icode{theTwistXPosArray\index{theTwistXPosArray}} and add it to
the accumulator, so that we can store our result in \icode{pixelXPosition\index{pixelXPosition}}.
...
\clearpage
\textbf{Lines 1144-1207. \icode{\textbf{CheckKeyboardInput\index{CheckKeyboardInput}}}} 
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,escapechar=\%]
;-------------------------------------------------------
...
\textbf{Lines 1181-1188. \icode{\textbf{SpacePressed}}:} This is the routine that detects when the player has selected a new
pattern by pressing the 'Space' key. It is part of the much larger routine \icode{CheckKeyboardInput\index{CheckKeyboardInput}} which periodically checks
...
Otherwise we increment \icode{currentPatternElement\index{currentPatternElement}}, which is the variable we use for storing the currently selected pattern.
Each press of the 'Space' key increments the value of \icode{currentPatternElement\index{currentPatternElement}} until it reaches 15. Once that happens, we 
clamp it back to zero by using an \icode{AND \#\$0F}.

\textbf{Lines 1190-1207. \icode{\textbf{UpdateCurrentPattern}}:} What is the series of \icode{ASL} instructions doing? A tricky
piece of business of course. We have loaded \icode{currentPatternElement\index{currentPatternElement}} to the \icode{A} register (\icode{LDA currentPatternElement\index{currentPatternElement}}) and it has a value between 0 and 8.
...
index given by \icode{Y} and write out the next 16 bytes to the screen, displaying the selected pattern briefly to the player.
\clearpage
\rhead[]{\leftmark}
...
\begin{figure}[H]
    ...
    \end{adjustbox}
\caption{
  A sketch from Minter\index{Minter}'s development notes of the multi-cross pattern. Compared with the final form on the previous page
...
\end{figure}
\vspace{1cm}
\begin{lstlisting}[caption=The paint order of pixels for 'La Llamita' in Psychedelia.,escapechar=\%]
laLlamitaXPosArray%\index{laLlamitaXPosArray}%  .BYTE $00,$FF,$00,$55                    ;  0       
                    .BYTE $00,$00,$55                        ; 06      
                    .BYTE $01,$02,$03,$00,$01,$02,$03,$55    ;  0      
                    .BYTE $04,$05,$06,$04,$00,$01,$02,$55    ;  1    3 
