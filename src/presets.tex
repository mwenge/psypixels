...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,caption=Data structure for Preset 0.,escapechar=\%]
;preset0
...
\clearpage
\textbf{Lines 980-994. \icode{\textbf{presetValueArray\index{presetValueArray}}}}
...
\textbf{Lines 179-182. \icode{\textbf{presetKeyCodes}}} 
\begin{lstlisting}[escapechar=\%]
presetKeyCodes
...
\end{lstlisting}
\textbf{Lines 1393-1405. \icode{\textbf{CheckIfPresetKeysPressed\index{CheckIfPresetKeysPressed}}}} 
\begin{lstlisting}[escapechar=\%]
;-------------------------------------------------------
...
\rhead[]{\icode{presetValueArray}}
\textbf{Lines 980-994. \icode{\textbf{presetValueArray\index{presetValueArray}}}:} Let's start at the end. This is where the values stored in the preset
...
\textbf{Lines 1393-1405. \icode{\textbf{CheckIfPresetKeysPressed}}:} Yet another entry in the long list of keyboard checks
that makes up \icode{CheckKeyboardInput\index{CheckKeyboardInput}}. This time we're checking if any of the 16 keys associated with a preset has been 
pressed.
...
time to load the preset data by calling \icode{DisplayPresetMessage\index{DisplayPresetMessage}}.
...
\clearpage
\textbf{Lines 2030-2070. \icode{\textbf{DisplayPresetMessage\index{DisplayPresetMessage}}}} 
\begin{lstlisting}[escapechar=\%]
;-------------------------------------------------------
...
\rhead[]{\icode{DisplayPresetMessage}}
\textbf{Lines 2030-2070. \icode{\textbf{DisplayPresetMessage\index{DisplayPresetMessage}}}:} Our objective here is modest enough. Display something like the following
...
\textbf{Lines 2037-2040. \icode{\textbf{WriteTemplateTextForPreset}}:} We'll split the task in two. First let's get some of the boiler plate text
...
\textbf{Lines 2053-2063. \icode{\textbf{WriteTemplateTextForPresetDigits}}:} Now we write the corresponding value for the selected preset. Remeber that
...
\textbf{Lines 2065-2066. \icode{\textbf{FinishedUpdatingDisplay}}:} When we've finished we'll now have something like this displayed on the screen.
\begin{figure}[H]                                                          
  \centering                                                             
...
We're ready to fill out the 'ACTIVATED' part and actually load the preset values. So let's \icode{JMP} to \icode{UpdateCurrentActivePreset\index{UpdateCurrentActivePreset}} and do that!
...
\clearpage
\rhead[]{\icode{UpdateCurrentActivePreset}}
\textbf{Lines 2085-2129. \icode{\textbf{UpdateCurrentActivePreset\index{UpdateCurrentActivePreset}}}} 
\begin{lstlisting}[escapechar=\%]
;-------------------------------------------------------
...
\textbf{Lines 2085-2129. \icode{\textbf{UpdateCurrentActivePreset\index{UpdateCurrentActivePreset}}}:} In the current path we're following (the one where we load a selected preset)
...
This is the final piece in \icode{statusLineBuffer\index{statusLineBuffer}} that we need to fill out
...
Later on we will actually write this buffer to the screen by copying all the values in \icode{statusLineBuffer\index{statusLineBuffer}} together to the last line on the screen:
...
For now though we still have to actually load the preset data. So we \icode{JMP} to \icode{RefreshPresetData\index{RefreshPresetData}} after we've finished looping through \icode{DisplayActivatedOrStored}.
...
\clearpage
\textbf{Lines 2134-2160. \icode{\textbf{RefreshPresetData\index{RefreshPresetData}}}} 
\begin{lstlisting}[escapechar=\%]
...
\rhead[]{\icode{RefreshPresetData}}
\textbf{Lines 2134-2160. \icode{\textbf{RefreshPresetData\index{RefreshPresetData}}}:} Our first order of business here is to get the location of the selected preset
in our storage. This is where storing the value of the of selected preset in \icode{X} will come in useful. 

We call \icode{GetPresetPointersUsingXRegister\index{GetPresetPointersUsingXRegister}} to get this done. We've encountered this design pattern 
previously: we use the value in X to iterate from the start of the preset data (given by \icode{presetSequenceData\index{presetSequenceData}} and stored in a pair of pointers
(\icode{presetSequenceData\index{presetSequenceData}\-LoPtr/preset\-SequenceDataHiPtr}. We then iterate from zero until we reach the value of the preset selected by the player, 
incrementing \icode{presetSequenceDataLoPtr\index{presetSequenceDataLoPtr}} by 32 bytes each time. Once we reach the value selected, our two pointers will contain the address
...
as the preset data we already have loaded. All of these checks end in the same outcome: we call \icode{LoadSelectedPresetSequence\index{LoadSelectedPresetSequence}} to actually load 
...
\textbf{Lines 2165-2183. \icode{\textbf{LoadSelectedPresetSequence\index{LoadSelectedPresetSequence}}}} 
\begin{lstlisting}[escapechar=\%]
...
\rhead[]{\icode{LoadSelectedPresetSequence}}
\textbf{Lines 2165-2183. \icode{\textbf{LoadSelectedPresetSequence\index{LoadSelectedPresetSequence}}}:} After all this trouble we go ahead and load the data using our pointers into 
\icode{presetValueArray\index{presetValueArray}}. As we saw at the start of the chapter the fact that our preset values are stored contiguously means that a simple loop
will do the bulk of the job. However there are two trailing items the \icode{currentPatternElement\index{currentPatternElement}} and \icode{currentSymmetrySetting\index{currentSymmetrySetting}} that we need
to pick out by name. 
